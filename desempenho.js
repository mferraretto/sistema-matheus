...
function renderEvolucao(diaria) {
  ...
  if (diarioEl) {
    const rows = dias
      .map(
        (d) =>
          `<tr><td>${formatarData(d)}</td><td class="text-green-600 font-bold">R$ ${diaria[d].toLocaleString('pt-BR')}</td></tr>`,
      )
      .join('');
  ...
  if (semanalEl) {
    const rows = Object.keys(semanal)
      .sort()
      .map(
        (s) =>
          `<tr><td>${s}</td><td class="text-green-600 font-bold">R$ ${semanal[s].toLocaleString('pt-BR')}</td></tr>`,
      )
      .join('');
    semanalEl.innerHTML = `<thead><tr><th>Semana</th><th>Vendas</th></tr></thead><tbody>${rows}</tbody>`;
  }
  if (mensalEl) {
    const rows = Object.keys(mensal)
      .sort()
      .map(
        (m) =>
          `<tr><td>${formatarMes(m)}</td><td class="text-green-600 font-bold">R$ ${mensal[m].toLocaleString('pt-BR')}</td></tr>`,
      )
      .join('');
...
function mudarAba(ev) {
  ...
  document
    ...
    .forEach((b) => b.classList.remove('bg-gray-200', 'text-gray-700'));
