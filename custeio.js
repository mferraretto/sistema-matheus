...
function calcularMDF() {
  ...
  function calcularPeca(alturaId, diametroId, chapaId, pecaNum) {
    ...
    if (!chapa || !chapa.comprimento || !chapa.largura || !chapa.preco)
      return 0;
...
function calcularMaoObra() {
  ...
  const maoIndireta =
    parseFloat(document.getElementById('maoIndireta').value) || 0;
  const quantidadeMes =
    parseFloat(document.getElementById('quantidadeMes').value) || 0;
...
function calcularResumoFinal() {
  const nomeProduto =
    document.getElementById('nomeProduto')?.value || 'Produto não identificado';
  const codigoProduto =
    document.getElementById('codigoProduto')?.value || 'Sem código';
  ...
  const custoIndiretoMensal =
    parseFloat(document.getElementById('custoIndiretoMensal')?.value) || 0;
  const qtdMensal =
    parseFloat(document.getElementById('qtdMensal')?.value) || 1;
  const qtdVendida =
    parseFloat(document.getElementById('qtdVendida')?.value) || 0;

  const custoIndiretoUnit = custoIndiretoMensal / qtdMensal;
  const custoTotalUnit =
    custoMDF + custoMao + custoEmbalagem + custoIndiretoUnit;
  ...
  const pontoEquilibrio20 =
    lucroUnit20 > 0 ? custoIndiretoMensal / lucroUnit20 : 0;
  const pontoEquilibrio25 =
    lucroUnit25 > 0 ? custoIndiretoMensal / lucroUnit25 : 0;
  const pontoEquilibrio30 =
    lucroUnit30 > 0 ? custoIndiretoMensal / lucroUnit30 : 0;
...
function exportarResumoPDF() {
  ...
  setTimeout(() => {
    ...
    const nomeProduto =
      document.getElementById('nomeProduto').value.trim() || 'Produto';
    const codigoProduto =
      document.getElementById('codigoProduto').value.trim() || 'codigo';
...
async function listarCalculos() {
  ...
  if (snapshot.empty) {
    tabela.innerHTML =
      '<tr><td colspan="4" style="text-align:center;">Nenhum cálculo salvo</td></tr>';
    return;
  }
  for (const doc of snapshot.docs) {
    const enc = doc.data().encrypted;
