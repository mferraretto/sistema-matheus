...
async function carregarSkus(uid) {
  ...
  const resumo = {};
  const mesesSnap = await getDocs(collection(db, `uid/${uid}/skusVendidos`));
  for (const mesDoc of mesesSnap.docs) {
    const listaSnap = await getDocs(collection(db, `uid/${uid}/skusVendidos/${mesDoc.id}/lista`));
    listaSnap.forEach(item => {
      const dados = item.data();
      const sku = dados.sku || 'sem-sku';
      const qtd = Number(dados.total) || 0;
