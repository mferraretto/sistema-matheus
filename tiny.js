...
document.querySelectorAll('.tabTiny').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    document
      .getElementById('tab-produtos')
      .classList.toggle('hidden', t !== 'produtos');
    document
      .getElementById('tab-pedidos')
      .classList.toggle('hidden', t !== 'pedidos');
    document
      .getElementById('tab-config')
      .classList.toggle('hidden', t !== 'config');
  });
});
...
document.getElementById('btnSyncPedidosPeriodo').onclick = async () => {
  try {
    ...
    if (!resp.ok || !data.ok)
      throw new Error(data.error || 'Falha no sync de pedidos');
    stateTiny.pedidos.lastDoc = null;
    stateTiny.pedidos.stack = [];
    stateTiny.pedidos.page = 1;
    carregarPedidos();
...
document.getElementById('btnConectarTiny').onclick = async () => {
  try {
    ...
    document.getElementById('tinyStatus').textContent =
      'Tiny conectado com sucesso.';
  } catch (e) {
...
document.getElementById('btnFiltrarProdutos').onclick = () => {
  stateTiny.produtos.search = document
...
async function carregarProdutos(direction = 0) {
  ...
  if (stateTiny.produtos.search) {
    ...
    q = query(
      ...
      startAt(s),
      endAt(s + '\uf8ff'),
      limit(stateTiny.produtos.pageSize),
    );
    ...
    q = query(
      ref,
      orderBy('nome'),
      startAfter(stateTiny.produtos.lastDoc),
      limit(stateTiny.produtos.pageSize),
    );
  } else if (direction === -1) {
    if (stateTiny.produtos.stack.length) {
      ...
      const startAfterDoc =
        stateTiny.produtos.stack[stateTiny.produtos.stack.length - 1];
      q = startAfterDoc
        ? query(
            ref,
            orderBy('nome'),
  ...
  document.getElementById('pagProdutos').textContent =
    `Página ${stateTiny.produtos.page}`;
}
...
async function carregarPedidos(direction = 0) {
  ...
  if (direction === 1 && stateTiny.pedidos.lastDoc) {
    ...
    q = query(
      ref,
      orderBy('data'),
      startAfter(stateTiny.pedidos.lastDoc),
      limit(stateTiny.pedidos.pageSize),
    );
  } else if (direction === -1) {
    if (stateTiny.pedidos.stack.length) {
      ...
      const startAfterDoc =
        stateTiny.pedidos.stack[stateTiny.pedidos.stack.length - 1];
      q = startAfterDoc
        ? query(
            ref,
            orderBy('data'),
  ...
  document.getElementById('pagPedidos').textContent =
    `Página ${stateTiny.pedidos.page}`;
}
...
function cardPedido(p) {
  const itens = (p.itens || [])
    .map(
      (i) =>
        `<li>${i.quantidade}x ${i.sku} — R$ ${(i.preco || 0).toFixed(2)}</li>`,
    )
    .join('');
